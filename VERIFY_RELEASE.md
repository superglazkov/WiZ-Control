# WiZ Control 3.4.4 release verification

This archive is a **complete source project**, not a binary installer. Build on Windows 11 x64 with Node.js LTS using `BUILD_WINDOWS.cmd` from this folder. The generated installer is `dist/WiZ-Control-3.4.4-Setup.exe`. It upgrades the existing `WiZ Control` identity without wiping AppData. Backup your `%APPDATA%` WiZ Control data before any removal.
...
3. Open Help/About and confirm `3.4.4`, `Superglazkov`, and the source link.
4. Check that Settings has no Spanish or Auto-Detect choices. Do not regress theme selection or lamp renaming.
